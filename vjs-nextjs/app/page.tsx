  ...
  <section data-testid="home-services" className="py-28 lg:py-40 bg-ink-900/30 border-y border-ink-800/80">
    <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16">
      ...
      <Stagger className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-l border-t border-ink-800/80">
        {SERVICES.map((s) => {
          const Icon = (Icons as unknown as Record<string, LucideIcon>)[s.icon] || Icons.Cpu;
          return (
        ...
        <StaggerItem>
          <div className="relative p-8 lg:p-10 border-r border-b border-ink-800/80 bg-signal-500 text-white flex flex-col justify-between min-h-[220px]">
            ...
            <Link href="/contact" data-testid="services-cta" className="inline-flex items-center gap-2 font-mono text-xs uppercase tracking-[0.2em] mt-6">
        ...
        {INDUSTRIES.map((ind, i) => (
          <StaggerItem key={ind.name}>
            <div
              ...
              className="group relative aspect-[4/5] overflow-hidden border border-ink-800 cursor-pointer"
            >
              <Image
                src={ind.img}
                alt={ind.name}
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-14 items-end">
          ...
          <div className="lg:col-span-3 lg:col-start-10 flex lg:justify-end">
            <Link href="/products" className="inline-flex items-center gap-2 border border-ink-700 hover:border-signal-500 hover:text-signal-500 px-6 py-3 font-mono text-xs uppercase tracking-[0.2em] transition-colors">
        ...
        <Stagger className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((p) => (
            <StaggerItem key={p.slug}>
              <Link href={`/products/${p.slug}`} data-testid={`featured-${p.slug}`} className="group block border border-ink-800 hover:border-signal-500 transition-colors">
...
const CTASection = () => (
  <section data-testid="home-cta" className="relative overflow-hidden">
    <div className="absolute inset-0">
      <Image src={IMAGES.ctaBanner} alt="" fill className="object-cover opacity-25" sizes="100vw" />
    ...
    <div className="relative max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16 py-28 lg:py-40">
      <div className="max-w-3xl">
        ...
        <div className="mt-12 flex flex-wrap gap-3">
          <Link href="/contact" data-testid="cta-contact" className="group inline-flex items-center gap-2 bg-signal-500 hover:bg-signal-600 text-white px-8 py-5 text-xs font-mono uppercase tracking-[0.2em]">
            Contact Us <ArrowUpRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
          </Link>
          <Link href="/contact" data-testid="cta-quote" className="inline-flex items-center gap-2 border border-ink-700 hover:border-signal-500 hover:text-signal-500 text-ink-100 px-8 py-5 text-xs font-mono uppercase tracking-[0.2em] transition-colors">
            Request a Quote
          </Link>
        </div>
      </div>
